...
\subsection*{March 2 : Building Bioreactors}

Talks, starting at 10:00:
\begin{itemize}
\item Welcome Message and Overview, 30 min, Rainer
\item Rob's DIY Reactor - \texttt{NinjaPBR} -
  Arduino-controlled mini PBR, Rob
%\item Dougie's DIY Reactor - 20 yrs Later
\item CellDeg - Optimizing Photosynthetic Growth, Rob
% : Introduction to
...
\item Introduction to the \gasometer{}: connecting sensors with Arduino,
  making an autonomous measurement device via Sainsmart's Touch Screen, Rainer
\item Introduction to 3D printing with QTB's \texttt{ultimaker$^2$}, Simon
...
%all projects.
Talk and Spectrometer Presentation, at 11:00, 90 min:
\begin{itemize}
...
%should we have skill badges, \href{https://www.adafruit.com/products/2857?q=skill\%20badge&p=0&}{like here}, or 3D-printed awards, like the ``red soldering iron'', ``the blue tube'', ``the green chip''?
...
\item \texttt{Should it be making that sound?}
\item \texttt{Ada Lovelace: Algorithm \& Combinatorics}
\item The \texttt{Bureaucrat}
\item The \texttt{DumpsterDIVER}
\item The \texttt{SkeuomorphER}
\item The \texttt{TroubleshootER}
\item The \texttt{Flash}
\end{itemize}
